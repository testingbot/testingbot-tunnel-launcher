...
async function startTunnelAsync (options = {}) {
    ...
    return new Promise((resolve, reject) => {
        ...
        activeTunnel.stderr.on('data', readStderr)
        activeTunnel.stderr.on('end', () => readStderr.flush())

        activeTunnel.stdout.on('data', readStdout)
        activeTunnel.stdout.on('end', () => readStdout.flush())
